...
library(tibble)
structure(list(table_name = c("concept_ancestor", "cb_criteria_ancestor", 
...
), columns = c("ancestor_concept_id, descendant_concept_id, min_levels_of_separation, max_levels_of_separation", 
...
The `allofus` package includes a function called `aou_join()` that makes it easy to join tables together. It includes some additional checks to help avoid mistakes in joining. For example, if we wanted to join the `person` table with the `observation` table, we could do it like this:
```{r}
person_tbl %>%
...
Understanding the SQL code that `dbplyr` generates can be insightful, especially if you're debugging or simply curious about the translation from R to SQL.

To view the SQL query that corresponds to your `dbplyr` operations, use the `show_query()` function:
...
This function prints the SQL query that would be sent to the database. It's a great way to learn SQL and understand how `dbplyr` optimizes data manipulation.

## Running SQL code directly

## Conclusion

`allofus` leverages `dbplyr` to enable efficient and intuitive access to SQL databases directly from R. This allows for powerful data manipulation while avoiding the pitfalls of loading large datasets into memory.
